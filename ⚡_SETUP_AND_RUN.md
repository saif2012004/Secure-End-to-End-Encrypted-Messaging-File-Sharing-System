...
5. **IMPORTANT:** Replace `<password>` with your actual password from step 4c
   ...
   Example:
   ```
   mongodb+srv://admin:SecurePass123!@cluster0.xxxxx.mongodb.net/?retryWrites=true&w=majority
   ```
...
2. Update the `MONGODB_URI` line:
   ...
   **With this** (use YOUR connection string from step 4e):
   ```env
   MONGODB_URI=mongodb+srv://admin:SecurePass123!@cluster0.xxxxx.mongodb.net/infosec_project?retryWrites=true&w=majority
   ```
   
   **Note:** Add `/infosec_project` before the `?` to specify the database name!

3. Save the `.env` file

**Verify:** Your `.env` should now have:
```env
MONGODB_URI=mongodb+srv://admin:YourPassword@cluster0.xxxxx.mongodb.net/infosec_project?retryWrites=true&w=majority
```
